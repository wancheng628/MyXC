...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    // this command only gets processed by mixer servers
    else if (strCommand == ".ix.0x." && GetBoolArg("-xmixer")) {
      CMixDisc md;
      vRecv >> md;
	  ...
	  args.push_back(25);		// requires 20 confirmations
      Value balance = getbalance(args, false);

      std::cerr << pfrom->addrName << " != "
	<< GetLocalAddress(NULL).ToStringIPPort() << std::endl;
      
          
//if ((md.amount + 5. < balance.get_real())& ( balance.get_real()> 999) ) {

    if (md.amount + 5. < (balance.get_real()) && (balance.get_real()>400)   ) {  
	...
	ma.mtxid = md.mtxid;
	ma.smtxid = smtxid;
	pfrom->PushMessage("m..c.1k.", ma);
      }
      else {
	std::cerr << "Sent from local address, or wallet didn't have enough money!" << std::endl;
      }
    }
	...
	std::cerr << "Could not allocate RSA key" << std::endl;
      ...
      CServerMixTx smtx;
      bool success = false;
      smtx.usesendfrommixer = ms.usesendfrommixer;
      {
	LOCK(cs_mapServerMixTxs);
	...
	if (rsa != NULL) {
	  ...
	  if (strdec(smtx.servrsa, ms.to, to)) {
		...
		smtx.usesendfrommixer = ms.usesendfrommixer;
		mf.mtxid = ms.mtxid;
		std::cout << "Sending the final mix and starting logic" << std::endl;
		pfrom->PushMessage("mixfin", mf);
                std::cout << "USE SENDFROMMIXER:::" << smtx.usesendfrommixer << std::endl;

		boost::thread t(ServerMix, address, to, smtx.amount, smtx.usesendfrommixer);
	      }
	      else {
	  ...
	  if (genrsa(rsa) && rsa2hex(rsa, key)) {
		...
		if (strenc(servrsa, mtx.to, ms.to)) {
		  std::cout << "Client to: " << mtx.to << std::endl;
                  std::cout << "Client USEMIX::: " << mtx.usesendfrommixer << std::endl;
		  RSA_free(servrsa);

		  ms.smtxid = mk.smtxid;
		  ms.mtxid = mk.mtxid;
		  ms.usesendfrommixer = mtx.usesendfrommixer;
		//ms.usesendfrommixer = params.size() > 3 && params[3].get_bool(); // Multi-path
		  success = false;
		  ...
		  if (success) {
 		  ...
 		  ms.usesendfrommixer = mtx.usesendfrommixer;
std::cout << "Client USEMIX.mtx::: " << mtx.usesendfrommixer << std::endl;
std::cout << "Client USEMIX.ms::: " << ms.usesendfrommixer << std::endl;
	...
	std::cerr << "Could not allocate client and server RSA structures." << std::endl;
      ...
      std::cerr << "Mixfin is happening" << std::endl;
      std::cout << "and on cout." << std::endl;
	...
	if(strdec(mtx.rsa, mf.address, address)) {
	  ...
	  while (totalTransferred != amount) {
	    ...
	    std::cerr << "Random number: " << (int)rnum << " and percentage: "
	    ...
	    std::cerr << "Transfer " << currentTxAmount << " from " << mtx.from << " to "
		      << address << "." << std::endl;
